...
const railLines = {
  ...
  'MEX16': {
    ...
    waypoints: [
      { lon: 11.5604, lat: 48.1408 }, // München Hbf
      { lon: 11.4000, lat: 48.1000 }, // München-Ost
      { lon: 11.0000, lat: 48.0500 }, // Grafing
      { lon: 10.5000, lat: 48.0000 }, // Wasserburg
      { lon: 10.0000, lat: 47.9500 }, // Mühldorf
    ],
    depots: [
      { lon: 10.8569, lat: 48.4908, name: 'Depot Langweid' },
      { lon: 10.0000, lat: 47.9500, name: 'Abstellung Mühldorf' }
    ]
  }
};

// Zugzustände mit realistischen Positionen
const trainStates = new Map<string, {
  isMoving: boolean;
...
function interpolatePosition(wp1: { lon: number; lat: number }, wp2: { lon: number; lat: number }, progress: number) {
  return {
