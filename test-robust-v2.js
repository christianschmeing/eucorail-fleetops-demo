...
class RobustTesterV2 {
  ...
  async startDemo() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkHanging = () => {
        const now = Date.now();
        if (now - lastActivity > 60000) { // 60 seconds without activity
  ...
  async testWithPuppeteer() {
    ...
    try {
      ...
      try {
        ...
        // Take screenshot
        await page.screenshot({ path: `test-result-attempt-${this.attempt + 1}.png`, fullPage: true });
        ...
        await page.screenshot({ path: `test-result-no-markers-attempt-${this.attempt + 1}.png`, fullPage: true });
        console.log(`📸 Screenshot saved as test-result-no-markers-attempt-${this.attempt + 1}.png`);
