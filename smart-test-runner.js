...
class SmartTestRunner {
  ...
  startHangDetection(process, name, timeout = 60000) {
    ...
    const checkHang = () => {
      ...
      if (now - lastOutput > timeout) {
        this.log(`Process ${name} appears to be hanging (no output for ${timeout/1000}s)`, 'error');
  ...
  async setupEnvironment() {
    ...
    // Create .env.local
    fs.writeFileSync('apps/web/.env.local', `
NEXT_PUBLIC_API_BASE=http://localhost:4100
API_ORIGIN=http://localhost:4100
    `.trim());
    ...
    while (this.currentStrategyIndex < this.strategies.length) {
      ...
      while (retries < this.maxRetries) {
        try {
          ...
          if (result.success) {
            this.log(`SUCCESS! Found ${result.markerCount} markers using ${strategy.name}`, 'success');
          ...
          this.log(`Strategy ${strategy.name} failed (attempt ${retries + 1}): ${error.message}`, 'error');
          await this.killEverything();
          await this.sleep(5000);
        }
