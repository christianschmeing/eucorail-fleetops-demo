...
export function ComplianceTracker() {
  ...
  // Vehicles nearing IS2/IS3 (>=75% usage of interval)
  const nearing: Array<{
    id: string;
    stage: 'IS2' | 'IS3';
    usagePct: number;
  ...
  vehicles.forEach((v: any) => {
    ...
    (['IS2', 'IS3'] as const).forEach((stage) => {
      const cfg = p[stage];
      if (!cfg) return;
      const kmSince = mileage % cfg.periodKm;
      const usage = (kmSince / cfg.periodKm) * 100;
      if (usage >= 75) {
        nearing.push({
          id: v.id,
          stage,
          usagePct: Math.round(usage),
          restKm: Math.max(0, cfg.periodKm - kmSince),
          restDays: Math.max(0, Math.round((cfg.periodDays || 0) * (1 - usage / 100))),
        });
      }
    });
  });
  ...
  return (
    <div className="space-y-6">
      {(criticalItems.length > 0 || nearing.length > 0) && (
        <Alert className="border-yellow-200 bg-yellow-50">
          <AlertTriangle className="h-4 w-4 text-yellow-600" />
          <AlertDescription className="text-yellow-900">
            <span className="font-semibold">
              {criticalItems.length} compliance items require attention
            </span>
            <ul className="mt-2 space-y-1">
              {criticalItems.map((item) => (
                <li key={item.id} className="text-sm">
                  • {item.title}: {item.standard} - Action required by{' '}
                  {new Date(item.nextAudit).toLocaleDateString()}
                </li>
              ))}
              {nearing.map((n) => (
                <li key={`${n.id}-${n.stage}`} className="text-sm">
                  • {n.id}: {n.stage} due soon — usage {n.usagePct}% (rest {n.restKm} km, ~
                  {n.restDays} Tage)
                </li>
              ))}
            </ul>
      ...
      <div className="grid grid-cols-2 gap-4">
        {complianceItems.map((item) => (
          <Card
            key={item.id}
            className={`border-l-4 ${item.status === 'compliant' ? 'border-l-green-500' : item.status === 'attention' ? 'border-l-yellow-500' : 'border-l-red-500'}`}
          >
            <CardBody className="space-y-4">
              <div className="flex items-start justify-between">
                ...
                <Badge className={getPillClass(item.status)}>
