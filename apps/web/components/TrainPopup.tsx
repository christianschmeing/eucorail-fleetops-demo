...
export default function TrainPopup({ trainId, onClose }: { trainId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    let alive = true;
    setLoading(true);
    fetch(`${api}/api/maintenance/${encodeURIComponent(trainId)}`)
      .then(async (r) => {
  ...
  const badge = (s: MaintItem['status']) =>
    s === 'ok'
  ...
  return (
    <div
      role="dialog"
      aria-label="Wartung & Zustand"
      className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[640px] bg-[#0B1F2A]/95 border border-[#2A3F4A] rounded-lg shadow-xl"
    >
      ...
      <div className="p-4 grid grid-cols-2 gap-4">
            ...
            <ul className="space-y-2">
              {data.faults.map((f) => (
                <li key={f.code + f.since} className="text-xs">
                  <div
                    className={`inline-block px-2 py-0.5 rounded border mr-2 ${f.severity === 'critical' ? 'bg-red-600/30 border-red-700 text-red-200' : f.severity === 'major' ? 'bg-yellow-600/30 border-yellow-700 text-yellow-200' : 'bg-white/10 border-white/20 text-white'}`}
                  >
                    {f.code}
                  </div>
                  <span className="text-gray-300">{f.message}</span>
                  <span className="text-gray-500 ml-2">
                    seit {new Date(f.since).toLocaleString()}
                  </span>
                </li>
              ))}
            </ul>
        ...
        <div>
          ...
          <div className="text-xs text-gray-300">
            {data.next.type} bei {data.next.atKm.toLocaleString()} km
          </div>
        </div>
        <div className="col-span-2">
          ...
          <table className="w-full text-xs">
            <thead className="text-gray-400">
              <tr>
                ...
                <th className="text-left">Letzter Service (km)</th>
                <th className="text-left">Fällig bei (km)</th>
            ...
            <tbody>
              {data.schedule.map((it) => (
                <tr key={it.component} className="border-t border-white/10">
                  ...
                  <td>
                    <span className={`px-2 py-0.5 rounded border ${badge(it.status)}`}>
                      {it.status}
                    </span>
                  </td>
                </tr>
          ...
          <div className="mt-3">
            <button
              ...
              onClick={() => window.open('https://www.eucorail.de', '_blank')}
            >
              Eucorail Handbuch (extern)
            </button>
          </div>
        </div>
      </div>
