...
export function SimpleSidebar({ trains = [], selectedTrain, onSelect }: SimpleSidebarProps) {
  ...
  return (
    <aside className="w-96 bg-gray-900/95 backdrop-blur-sm border-r border-gray-700 h-full flex flex-col" data-testid="sidebar">
      {/* Header */}
          ...
          {['active','standby','maintenance'].map((s) => (
            <button
              ...
              className={`px-3 py-1 rounded border text-xs ${
                statusFilter === s ? 'bg-blue-900/50 border-blue-700 text-blue-300' : 'bg-gray-800 border-gray-700 text-gray-300'
              }`}
            >
              {s}
            </button>
        ...
        <div role="group" aria-label="Region" className="flex items-center gap-2">
          {['BY','BW'].map((r) => (
            <button
              ...
              className={`px-3 py-1 rounded border text-xs ${
                regionFilter === r ? 'bg-purple-900/50 border-purple-700 text-purple-300' : 'bg-gray-800 border-gray-700 text-gray-300'
              }`}
            >
              {r}
            </button>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="text-sm text-gray-400 mb-2">
          {filteredTrains.length} Züge gefunden
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500 space-y-1">
                    {train.depot && (
                      <p>Depot: <span className="text-gray-400">{train.depot}</span></p>
                    )}
                    {train.speed !== undefined && train.speed > 0 && (
                      <p>Geschwindigkeit: <span className="text-blue-400">{train.speed} km/h</span></p>
                    )}
                    {train.nextStop && (
                      <p>Nächster Halt: <span className="text-gray-400">{train.nextStop}</span></p>
                    )}
                  </div>
                </div>
