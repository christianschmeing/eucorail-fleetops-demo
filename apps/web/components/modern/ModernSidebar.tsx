...
export function ModernSidebar({
  ...
  const cardHoverStyle =
    'hover:bg-white/5 hover:border-white/20 hover:scale-[1.02] hover:shadow-2xl';

  // Status-Farben mit Neon-Glow
  const statusColors = {
    active:
      'bg-emerald-500/20 text-emerald-400 border-emerald-500/30 shadow-[0_0_20px_rgba(52,211,153,0.5)]',
    standby:
      'bg-amber-500/20 text-amber-400 border-amber-500/30 shadow-[0_0_20px_rgba(251,191,36,0.5)]',
    maintenance:
      'bg-red-500/20 text-red-400 border-red-500/30 shadow-[0_0_20px_rgba(239,68,68,0.5)]',
  };
  ...
  const filteredTrains = trains.filter((train) => {
    ...
    const matchesStatus = selectedStatus.length === 0 || selectedStatus.includes(train.status);

    const matchesLine = activeLines.length === 0 || activeLines.includes(train.lineId);

    return matchesSearch && matchesStatus && matchesLine;
  });

  // Gruppiere nach Linien
  const groupedTrains = filteredTrains.reduce(
    (acc, train) => {
  ...
  return (
    <aside
      className={`w-96 ${glassStyle} h-full overflow-hidden flex flex-col`}
      data-testid="sidebar"
    >
      {/* Header mit Gradient */}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-white/50 space-y-1">
                      {train.nextStop && (
                        <p>
                          Nächster Halt: <span className="text-white/70">{train.nextStop}</span>
                        </p>
                      )}
