...
export default function ECMMaintenanceClient({
  ...
  const overdueWOs = workOrders.filter(
    (wo) => wo.status !== 'COMPLETED' && new Date(wo.dueDate) < new Date()
  ).length;

  // Fahrzeugtyp-Verteilung in Wartung
  const maintenanceByVehicleType = workOrders.reduce(
    (acc, wo) => {
      ...
      <div className="p-6 space-y-6">
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <div className={`text-2xl font-bold text-${info.color}-400`}>{info.name}</div>
                    <div className="text-sm text-gray-400 mt-1">{info.label}</div>
                  </div>
                ...
                <div className="text-xs text-gray-500">
                  {count > 0 && (
                    <div className="space-y-1 mt-3">
                      {woByECMLevel[levelNum as keyof typeof woByECMLevel].slice(0, 3).map((wo) => (
                ...
                {Object.entries(vehicleTypes).map(([type, count]) => {
                  ...
                  return (
                    <div key={type} className="flex justify-between items-center">
                      <span className={`text-sm ${typeInfo.text}`}>{typeInfo.label}</span>
                      <span className="text-sm text-gray-400">{count} Züge</span>
                    </div>
                  );
                })}
              </div>
                ...
                {riskParts.slice(0, 5).map((part) => (
                  <tr key={part.partId} className="hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-300">
                      <span className="font-semibold">{part.affectedTrains.length}</span>/
                      {fleetSize}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-300">
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white mb-4">
            IHB-Profile & Wartungsintervalle
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { key: 'FLIRT3', title: 'Stadler FLIRT', color: 'blue' },
              { key: 'MIREO', title: 'Siemens Mireo H2', color: 'green' },
              { key: 'DESIRO_HC', title: 'Siemens Desiro HC', color: 'purple' },
            ].map(({ key, title, color }) => {
              const prof: any = (ECM_PROFILES as any)[key];
              if (!prof) return null;
              const rows: Array<{ label: string; d?: number; km?: number }> = [
                { label: 'IS1', d: prof.IS1?.periodDays, km: prof.IS1?.periodKm },
                { label: 'IS2', d: prof.IS2?.periodDays, km: prof.IS2?.periodKm },
                { label: 'IS3', d: prof.IS3?.periodDays, km: prof.IS3?.periodKm },
                { label: 'IS4', d: prof.IS4?.periodDays, km: prof.IS4?.periodKm },
              ];
              return (
                <div
                  key={key}
                  className={`bg-${color}-900/20 border border-${color}-800/50 rounded-lg p-4`}
                >
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-400">Radreprofilierung:</span>
                      <span className="text-gray-300">{prof.WHEEL_LATHE?.periodKm ?? '-'} km</span>
                    </div>
                  </div>
                </div>
              );
