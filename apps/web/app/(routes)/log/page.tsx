...
function generateEventsFor144Trains(): LogEvent[] {
  ...
  const eventTypes: LogEvent['type'][] = [
    'PolicyApproved', 'WorkOrderClosed', 'WorkOrderAssigned', 'MaintenancePlanUpdated',
    'PositionStreamConnected', 'AlarmRaised', 'AlarmCleared', 'UserLogin',
    'SignoffCompleted', 'DataVersionTagged', 'DEPOT_PLAN_UPDATE', 'DEPOT_RELEASE',
    'DEPOT_ALLOCATION'
  ];
  ...
  const detailsMap: Record<LogEvent['type'], string[]> = {
    PolicyApproved: ['ECM-Richtlinie 2025 freigegeben', 'Sicherheitsprotokoll aktualisiert', 'Wartungsintervall angepasst'],
    WorkOrderClosed: ['Bremstest abgeschlossen', 'Softwareupdate installiert', 'Türmechanik geprüft', 'Klimaanlage gewartet'],
    WorkOrderAssigned: ['Techniker Schmidt zugeteilt', 'Team Stuttgart übernimmt', 'Priorität erhöht'],
    MaintenancePlanUpdated: ['Intervall auf 30 Tage gesetzt', 'Zusatzprüfung eingeplant', 'Winterwartung aktiviert'],
    ...
    SignoffCompleted: ['ECM-4 Freigabe erteilt', 'Qualitätskontrolle bestanden', 'Inspektion abgezeichnet'],
    DataVersionTagged: ['v2.4.1 deployed', 'Backup erstellt', 'Konfiguration gesichert'],
    DEPOT_PLAN_UPDATE: ['Gleis E-H1 neu zugewiesen', 'IS3-Wartung eingeplant', 'Zeitfenster verschoben'],
    ...
    DEPOT_ALLOCATION: ['Zug auf Gleis L-H3 eingeplant', 'Reserve aktiviert', 'Notfall-Slot zugewiesen'],
  ...
  for (let i = 0; i < 50; i++) {
    ...
    events.push({
      ...
      details: ['Backup abgeschlossen', 'Datenbank optimiert', 'Cache geleert', 'Logs rotiert', 'Metriken aggregiert'][i % 5]
    });
  }
