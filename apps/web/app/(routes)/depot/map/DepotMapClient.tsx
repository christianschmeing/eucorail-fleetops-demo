...
const DepotMap = dynamic(() => import('./DepotMap'), { 
  ssr: false,
  loading: () => (
...
export default function DepotMapClient({
  ...
  const filteredAllocations = useMemo(() => {
    let filtered = allocations.filter(a => 
      filteredTracks.some(t => t.id === a.trackId)
    );
  ...
  const handleDrop = useCallback((trainId: string, trackId: string) => {
  ...
  const handleExportCSV = () => {
    const BOM = '\uFEFF';
    const headers = ['Depot', 'Gleis', 'Zug-ID', 'Linie', 'IS-Stufe', 'Start', 'Ende', 'ETA-Freigabe', 'Status', 'Reserve', 'Risiko'];
          ...
          <div className="flex gap-4">
            <div className="text-right">
              ...
              <div className="text-xl font-bold text-white">
                {selectedDepot === 'Essingen' ? kpis.trainsInDepot.essingen : kpis.trainsInDepot.langweid}/{kpis.fleetSize}
              </div>
            </div>
            <div className="text-right">
            ...
            <div className="text-right">
              ...
              <div className={`text-xl font-bold ${conflicts.length > 0 ? 'text-red-400' : 'text-green-400'}`}>
                {conflicts.length}
              </div>
            </div>
            <div className="text-right">
            ...
            <DepotMapInspector
              ...
              onConflictClick={(conflict: any) => {
                const allocation = allocations.find(a => 
                  conflict.trainIds.includes(a.train_id)
                );
                if (allocation) {
