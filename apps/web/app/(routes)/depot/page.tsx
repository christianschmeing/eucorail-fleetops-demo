...
const TRACKS: Track[] = [
  ...
  },
  {
    ...
    depot: 'Langweid',
    length: 220,
    features: ['Halle', 'OL', 'Radsatzdrehmaschine'],
    status: 'Belegt',
  },
  {
    id: 'L-H4',
...
function loadFleetData(): Array<{
  ...
  for (let i = 1; i <= 13; i++) {
    fleetData.push({
      ...
      homeDepot: 'Langweid',
      isReserve: true,
      status: i === 12 ? 'maintenance' : 'reserve',
    });
  }
...
function generateAllocations(): Allocation[] {
  ...
  for (let i = 1; i <= 144; i++) {
    ...
    const lineCode =
      i <= 59
        ? ['RE1', 'RE2', 'RE8', 'RB22', 'RB27'][i % 5]
        : i <= 108
          ? ['RE9', 'RE12', 'MEX16', 'MEX18', 'MEX12', 'RB32', 'RB54'][i % 7]
          : i <= 122
            ? ['S2', 'S3', 'S4', 'S6'][i % 4]
            : 'RESERVE';
  ...
  for (let day = 0; day < DAYS_TO_PLAN; day++) {
    const trainsPerDay =
      Math.floor(TARGET_TRAINS / DAYS_TO_PLAN) + (day < TARGET_TRAINS % DAYS_TO_PLAN ? 1 : 0);
    ...
    todaysTrains.forEach((train, idx) => {
      ...
      if (track && allocationId <= 250) {
        // Maximal 250 Allocations generieren
        ...
        const riskLevel: Allocation['riskLevel'] =
          etaDiff < 30 * 60 * 1000 ? 'high' : etaDiff < 60 * 60 * 1000 ? 'medium' : 'low';
        ...
        allocations.push({
          ...
          lineId,
          isLevel,
          tasks: train.isReserve
            ? ['Reserve-Bereitstellung', 'Systemcheck', 'Verfügbarkeit']
...
function generateConflicts(allocations: Allocation[]): Conflict[] {
  ...
  if (allocations.filter((a) => a.team === 'Team Alpha' && a.status === 'In Arbeit').length > 3) {
    conflicts.push({
      ...
      trackId: 'E-G1',
      trainIds: allocations
        .filter((a) => a.team === 'Team Alpha')
        .map((a) => a.trainId)
        .slice(0, 4),
...
export default async function DepotPage() {
  ...
  const occupiedTracks = new Set(
    allocations.filter((a) => a.status === 'In Arbeit').map((a) => a.trackId)
  ).size;
  ...
  const avgStandingTime =
    allocations.reduce((sum, a) => {
      const duration =
        (new Date(a.endTime).getTime() - new Date(a.startTime).getTime()) / (1000 * 60 * 60);
      return sum + duration;
